// Usando require para express e prom-client corretamente
const express = require('express');  // Importando o express
const client = require('prom-client');  // Importando o prom-client
...
const app = express();  // Inicializando o objeto 'app' do Express

// Crie um histograma para medir a duração das requisições HTTP
const httpRequestDurationMicroseconds = new client.Histogram({
  name: 'http_request_duration_ms',  // Nome da métrica
  help: 'Duração das requisições HTTP em ms',  // Descrição da métrica
  labelNames: ['method', 'route', 'code'],  // Labels para categorizar a métrica
  buckets: [50, 100, 200, 300, 400, 500, 750, 1000]  // Definindo os baldes para agrupar as durações
});

// Contador para número de requisições HTTP
...
// Middleware para medir o tempo de resposta das requisições HTTP
app.use((req, res, next) => {
  const end = httpRequestDurationMicroseconds.startTimer();
  res.on('finish', () => {
    end({ method: req.method, route: req.route ? req.route.path : '', code: res.statusCode });
    httpRequestCount.inc({ method: req.method, route: req.route ? req.route.path : '', code: res.statusCode });
  });
  next();
});

// Expor o endpoint de métricas em /metrics
...
// Defina a porta onde o servidor de métricas rodará
const metricsPort = 9100;
app.listen(metricsPort, () => {
  console.log(`Servidor de métricas rodando na porta ${metricsPort}`);
});


// Imprimir versão da aplicação
console.log(
  `App version: ${
...
require("./react-components/styles/global.scss");
require("./assets/stylesheets/globals.scss");
require("./assets/stylesheets/hub.scss");
const loadingEnvironment = require("./assets/models/LoadingEnvironment.glb");
